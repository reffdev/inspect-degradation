...
**Step validity is not fully decidable from trace data alone.** The grader judges each step using only the context available *before* that step (no hindsight). This is the right construct for measuring decision quality, but it creates an irreducible gap: an edit that compiles and looks correct at step 3 but turns out wrong when a test fails at step 60 is a *good decision* under the grader's construct but a *bad outcome contribution* under the construct a trace-aware annotator naturally applies. These are fundamentally different things, and no amount of improved grading can collapse them — separating decision quality from outcome contribution requires knowing the future. The [companion study](https://github.com/reffdev/inspect-degradation-study) finds this construct mismatch is the dominant source of grader-vs-human disagreement, ahead of conventional grading errors. Users should interpret "validity" as "reasonable given available information," not "actually correct."
